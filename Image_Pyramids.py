...
def main():
    ...
    parser.add_argument('--image_path', type=str, default='./data', help='Raw images path')
    parser.add_argument('--result_path', type=str, default='./result', help='Blending images path')
    ...
    dst_list=[s for s in os.listdir(image_path) if 'dst' in s]
    src_list=[s for s in os.listdir(image_path) if 'src' in s]
    mask_list=[s for s in os.listdir(image_path) if 'Mask' in s]

    for dst_img in dst_list:
        src_img=[s for s in src_list if dst_img.split('_dst.jpg')[0] in s][0]
        mask_img=[s for s in mask_list if dst_img.split('_dst.jpg')[0] in s][0]
        image1 = cv2.imread(os.path.join(image_path,dst_img))
        image2 = cv2.imread(os.path.join(image_path,src_img))
        mask = cv2.imread(os.path.join(image_path,mask_img))
        ...
        cv2.imwrite(os.path.join(result_path,'{}_blending_img.jpg'.format(dst_img.split('_dst.jpg')[0])), result)
